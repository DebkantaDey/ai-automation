...
let EmailService = EmailService_1 = class EmailService {
    ...
    provider;
    frontendUrl;
    constructor(configService, consoleProvider, smtpProvider) {
        this.configService = configService;
        this.consoleProvider = consoleProvider;
        this.smtpProvider = smtpProvider;
        this.frontendUrl = this.configService.get('app.frontendUrl') || process.env.FRONTEND_URL || 'http://localhost:3000';
        const emailHost = this.configService.get('EMAIL_HOST') || process.env.EMAIL_HOST;
        if (emailHost) {
    ...
    async sendInvitationEmail(to, inviterName, orgName, roleName, token) {
    ...
    async sendSecurityAlertEmail(to, name, message) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
