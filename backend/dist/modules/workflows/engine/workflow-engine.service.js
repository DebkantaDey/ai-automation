...
let WorkflowEngineService = WorkflowEngineService_1 = class WorkflowEngineService {
    ...
    constructor(executionModel, aiGateway, integrationsService, eventBus) {
    ...
    async executeNode(node, context, aiUsage) {
        ...
        switch (node.type) {
            ...
            case 'http_request': {
                ...
                if (!url) {
                    throw new common_1.BadRequestException(`HTTP node [${node.label || node.id}] missing destination URL`);
                }
                try {
            ...
            case 'loop': {
                ...
                const errorPolicy = data.errorPolicy || 'continue_on_error';
            ...
            case 'ai_decision': {
                ...
                let parsedOutput = aiResponse.text;
            ...
            case 'action_discord': {
                ...
                if (!connectionId) {
                    throw new common_1.BadRequestException(`Integration step [${node.label || node.id}] requires a selected Connection`);
                }
    ...
    async runWorkflow(executionId, nodes, edges, initialPayload = {}, resumeFromNodeId) {
        ...
        const aiUsage = execution.aiUsage || {
            promptTokens: 0,
            completionTokens: 0,
            totalTokens: 0,
            costUsd: 0,
    ...
    async approveExecution(executionId, actorUserId, reason) {
        ...
        if (execution.status !== 'waiting_approval') {
            throw new common_1.BadRequestException(`Execution is in status '${execution.status}', not waiting for approval`);
        }
    ...
    async rejectExecution(executionId, actorUserId, reason) {
        const execution = await this.executionModel.findById(this.toObjectId(executionId));
        if (!execution) {
            throw new common_1.NotFoundException('Execution record not found');
        }
