    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b border-neutral-200 pb-5">
        ...
        <Button
          ...
          className="gap-1.5 text-xs bg-neutral-900 hover:bg-neutral-800 text-white font-semibold shadow-sm"
        >
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-neutral-100 text-[11px] text-neutral-400">
                        <div className="flex flex-wrap gap-1">
                          {agent.tools?.map((tool: any) => (
                            <span
                              key={tool.name}
                              className="text-[10px] bg-neutral-100 text-neutral-700 px-1.5 py-0.5 rounded font-mono"
                            >
                              {tool.name}
                            </span>
                          ))}
          ...
          {selectedAgent ? (
            <Card className="border-neutral-200 flex flex-col h-full">
              ...
              <CardContent className="p-5 space-y-4 flex-1 flex flex-col">
                ...
                <form onSubmit={handleRunAgent} className="space-y-2">
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="text-xs bg-neutral-900 hover:bg-neutral-800 text-white gap-1.5 shrink-0 font-semibold"
                    >
                ...
                <div className="flex-1 min-h-[380px] rounded-xl border border-neutral-200 bg-neutral-900 p-4 space-y-3 overflow-y-auto max-h-[520px] font-mono text-xs text-neutral-300">
                  {!currentExecution && !isRunning && (
                    <div className="h-full flex flex-col items-center justify-center text-neutral-400 text-xs py-20 font-sans">
                      <Terminal className="h-8 w-8 mb-2 opacity-40 text-neutral-400" />
                      <p>Enter a prompt above to observe the agent's real-time ReAct loop (Thought → Action → Observation).</p>
                    </div>
                  )}

                  {isRunning && (
                    <div className="flex items-center gap-2 text-xs text-neutral-300 py-8 justify-center animate-pulse">
                      <Activity className="h-4 w-4 animate-spin" />
                      <span>Agent executing ReAct cycle (Thought → Tool Invocation → Observation)...</span>
                    </div>
                  )}
                  ...
                  {currentExecution && (
                    <div className="space-y-3.5">
                      <div className="flex items-center justify-between border-b border-neutral-800 pb-2 text-[11px] text-neutral-400 font-sans">
                        ...
                        <span>Tokens: <strong className="font-mono text-neutral-300">{currentExecution.aiUsage?.totalTokens || 0}</strong></span>
                      </div>

                      {/* Step Traces */}
                      {currentExecution.steps?.map((step: any) => (
                        <div key={step.stepNumber} className="p-3.5 rounded-lg bg-neutral-950 border border-neutral-800 space-y-2 text-xs">
                          ...
                          {step.toolCall && (
                            <div className="p-2.5 rounded bg-black/60 border border-neutral-800 text-[11px] space-y-1">
                              <span className="text-neutral-200 font-bold">🛠 Tool Invocation: {step.toolCall.name}</span>
                              <pre className="text-[10px] text-neutral-400 overflow-x-auto">{JSON.stringify(step.toolCall.input, null, 2)}</pre>
                            </div>
                          )}

                          {step.observation && (
                            <div className="p-2.5 rounded bg-neutral-900 border border-neutral-800 text-[11px] space-y-1">
                      ...
                      {currentExecution.finalOutput && (
                        <div className="p-4 rounded-xl bg-neutral-800 border border-neutral-700 space-y-2 font-sans">
                          <div className="flex items-center justify-between">
                            ...
                            <button
                              ...
                              className="text-[10px] flex items-center gap-1 text-neutral-300 hover:text-white cursor-pointer"
                            >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 space-y-4 border border-neutral-200 max-h-[90vh] overflow-y-auto animate-in fade-in-0 zoom-in-95">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="p-1.5 rounded-lg bg-neutral-100 text-neutral-800">
                  <Bot className="h-4 w-4" />
                </div>
                <h2 className="text-sm font-bold text-neutral-900">Deploy Autonomous AI Agent</h2>
              </div>
              <button onClick={() => setShowCreateModal(false)} className="text-neutral-400 hover:text-neutral-600 text-sm cursor-pointer">
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleCreateAgent} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-neutral-700">Role & Scope</label>
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-neutral-700">System Persona Instructions *</label>
                <textarea
                  required
                  rows={4}
                  ...
                  className="w-full rounded-lg border border-neutral-200 bg-white p-2.5 text-xs text-neutral-900 focus:outline-none focus:border-neutral-900 leading-relaxed font-mono"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-neutral-700">AI Model</label>
                  <select
                    value={model}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-neutral-700">Max Step Cap (Safety)</label>
              ...
              <div className="space-y-2 pt-1">
                <label className="text-xs font-semibold text-neutral-700">Equipped Tools</label>
                <div className="grid grid-cols-2 gap-2">
                  {AVAILABLE_TOOLS.map((t) => (
                    <label key={t.name} className="flex items-center gap-2 text-xs text-neutral-700 cursor-pointer p-2 rounded-lg border border-neutral-200 hover:bg-neutral-50 transition-colors">
                      <input
                        type="checkbox"
