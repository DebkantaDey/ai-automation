      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-4 border-neutral-200">
          ...
          <div className="mt-3">
            ...
            <p className="text-[11px] text-neutral-600 font-semibold mt-1">100% Confirmation Rate</p>
          </div>
        </Card>
        ...
        <Card className="p-4 border-neutral-200">
          ...
          <div className="mt-3">
            ...
            <p className="text-[11px] text-neutral-600 font-semibold mt-1">Booked via WhatsApp & Webhooks</p>
          </div>
        </Card>
        ...
        <Card className="p-4 border-neutral-200">
          ...
          <div className="mt-3">
            <span className="text-2xl font-bold font-mono text-neutral-900">2.1%</span>
            <p className="text-[11px] text-neutral-600 font-semibold mt-1">Reduced by auto-reminders</p>
          </div>
        </Card>
      </div>
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-neutral-100">
            {appointments.map((apt) => (
              <div key={apt.id} className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 hover:bg-neutral-50">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-xs text-neutral-900">{apt.title}</h3>
                    <Badge variant={apt.status === 'confirmed' ? 'success' : 'default'} className="text-[9px] uppercase font-mono">
                      {apt.status}
                    </Badge>
                    {apt.isAiScheduled && (
                      <span className="text-[10px] text-neutral-600 font-mono font-semibold flex items-center gap-1">
                  ...
                  <p className="text-[11px] text-neutral-500">
                    Client: <strong className="text-neutral-700">{apt.customerName}</strong> ({apt.company}) • Assigned Staff: {apt.staff}
                  </p>
                </div>
